                ...
                <div className='max-w-7xl mx-auto'>
                    ...
                    <div className='bg-white rounded-lg shadow-md p-6 mb-6'>
                        ...
                        <p className='text-xs text-gray-500 mt-1'>
                            🔑 Token akan otomatis diambil saat test - tidak
                            perlu input manual!
                        </p>
                    </div>
                    ...
                    <div className='bg-white rounded-lg shadow-md p-6 mb-6'>
                        ...
                        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                            ...
                            <button
                                onClick={() =>
                                    testAPI(
                                        '/api/agent/messages?filter=unread',
                                        'Get Unread Messages',
                                    )
                                }
                                disabled={loading}
                            ...
                            <button
                                onClick={() =>
                                    testAPI(
                                        '/api/agent/messages?filter=today',
                            ...
                            <button
                                onClick={() =>
                                    testAPI(
                                        '/api/agent/messages?filter=unread&limit=5',
                                        'Get 5 Unread Messages',
                                    )
                                }
                                disabled={loading}
                            ...
                            <button
                                onClick={() => {
                                    ...
                                    if (ticketId) {
                                        testAPI(
                                            `/api/agent/messages?ticketId=${ticketId}`,
                                            `Get Messages from Ticket: ${ticketId}`,
                                        )
                                    }
                                }}
                                disabled={loading}
                    ...
                    {response && (
                        <div className='space-y-4'>
                            {/* Response Summary */}
                            <div
                                className={`rounded-lg p-6 ${response.ok ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}
                            >
                                <div className='flex items-center justify-between mb-4'>
                                    <h3
                                        className={`text-xl font-semibold ${response.ok ? 'text-green-800' : 'text-red-800'}`}
                                    >
                                        {response.ok
                                            ? '✅ Success'
                                            : '❌ Failed'}
                                    ...
                                    <div className='text-right'>
                                        <div
                                            className={`text-sm font-medium ${response.ok ? 'text-green-700' : 'text-red-700'}`}
                                        >
                                ...
                                <div className='bg-white rounded-lg shadow-md p-6'>
                                    ...
                                    0 ? (
                                        ...
                                        <div className='space-y-3 max-h-96 overflow-y-auto'>
                                            {response.data.data.messages.map(
                                                (msg, index) => (
                                                    <div
                                                        key={msg.messageId}
                    ...
                    <div className='bg-yellow-50 border border-yellow-200 rounded-lg p-6 mt-6'>
                        ...
                        <ul className='list-disc list-inside space-y-1 text-sm text-yellow-800'>
                            ...
                            <li>
                                Token otomatis diambil dari session login -
                                tidak perlu input manual!
                            </li>
                            <li>
                                Buka Browser Console (F12) untuk lihat detail
                                request/response
                            </li>
                            <li>
                                Test berbeda filter untuk lihat perbedaan hasil
                            </li>
                            <li>
                                Jika tidak ada messages, buat ticket dulu
                                sebagai customer
                            </li>
                        </ul>
                    </div>
