...
export default function AgentRequestDetail() {
    ...
    const fetchRequestData = async () => {
        ...
        try {
            ...
            // Fetch messages
            const messagesRes = await fetch(`/api/request/${requestId}/messages`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            })
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            ...
            const response = await fetch(`/api/agent/requests/${requestId}/messages`, {
                method: 'POST',
                headers: {
                            ...
                            {request && (
                                <div className="bg-white rounded-lg shadow-md p-6 mb-6">
                                    ...
                                    <div className="flex items-center gap-4 text-sm text-gray-500">
                                        ...
                                        <span>•</span>
                                        <span>Created: {new Date(request.createdAt).toLocaleString('id-ID')}</span>
                                    </div>
                                </div>
                            )}
                                ...
                                <div className="h-96 overflow-y-auto p-4 space-y-4">
                                    {messages.length === 0 ? (
                                        ...
                                        messages.map((msg, index) => (
                                            <div
                                                key={msg.id || index}
                                                className={`flex ${msg.sender === 'agent' || msg.senderRole === 'agent' ? 'justify-end' : 'justify-start'}`}
                                            >
                                                <div
                                                    className={`max-w-xs lg:max-w-md px-4 py-2 rounded-lg ${
                                                        msg.sender === 'agent' || msg.senderRole === 'agent'
